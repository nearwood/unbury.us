...
ResultsController.results = function() {
	...
	if (results == null) {
		alert("Consider purchasing enough life insurance to cover the debt you leave behind.");
	}
	else {
...
ResultsController.sort_loans = function(remaining_loans, current_principal, current_interest, payment_type) {
	if (payment_type == "snowball") {
		return Object.keys(remaining_loans).sort(function(a, b) {
			return remaining_loans[a].currentBalance - remaining_loans[b].currentBalance;
		});
	}
	...
	else if (payment_type == "avalanche2") {
		return Object.keys(remaining_loans).sort(function(a, b) {
			return (remaining_loans[b].interestRate * 0.000833333333333333 * remaining_loans[b].currentBalance) - (remaining_loans[a].interestRate * 0.000833333333333333 * remaining_loans[a].currentBalance);
		});
	}
	else {
